...
router = APIRouter(tags=["Finances"], dependencies=[Depends(get_current_admin_or_operational)])

# --- Resource Sources ---
@router.post("/resource-sources/", response_model=ResourceSourceResponse)
def create_resource_source(source: ResourceSourceCreate, db: Session = Depends(database.get_db)):
...
def read_resource_source(source_id: int, db: Session = Depends(database.get_db)):
...
def update_resource_source(source_id: int, source: ResourceSourceUpdate, db: Session = Depends(database.get_db)):
...
def create_wallet(wallet: WalletCreate, db: Session = Depends(database.get_db)):
    return FinanceService.create_wallet(db, wallet)

@router.get("/wallets/", response_model=List[WalletResponse])
def read_wallets(db: Session = Depends(database.get_db)):
    return FinanceService.get_wallets(db)

@router.get("/wallets/{wallet_id}", response_model=WalletResponse)
def read_wallet(wallet_id: int, db: Session = Depends(database.get_db)):
    return FinanceService.get_wallet_by_id(db, wallet_id)

@router.put("/wallets/{wallet_id}", response_model=WalletResponse)
def update_wallet(wallet_id: int, wallet: WalletUpdate, db: Session = Depends(database.get_db)):
...
def get_wallet_dashboard(wallet_id: int, db: Session = Depends(database.get_db)):
    return FinanceService.get_wallet_dashboard(db, wallet_id)

@router.get("/wallets/{wallet_id}/export")
def export_wallet_data(wallet_id: int, db: Session = Depends(database.get_db)):
    return FinanceService.export_wallet_data(db, wallet_id)

@router.post("/wallets/transfer", status_code=201)
def transfer_funds(transfer: TransferCreate, db: Session = Depends(database.get_db)):
...
def create_revenue(revenue: RevenueCreate, db: Session = Depends(database.get_db)):
...
def update_revenue(revenue_id: int, revenue_update: RevenueUpdate, db: Session = Depends(database.get_db)):
