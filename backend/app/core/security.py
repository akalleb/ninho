...
def encrypt_data(value: Optional[str]) -> Optional[str]:
    if value is None:
        return None
    if not fernet:
        return value
...
def decrypt_data(value: Optional[str]) -> Optional[str]:
    if value is None:
        return None
    if not fernet:
        return value
...
async def get_current_user(
    ...
    if supabase_user:
        ...
        user = db.query(models.Professional).filter(models.Professional.email == email).first()
    else:
        token_data = decode_token(token)
        if token_data is None:
            raise credentials_exception
        user = db.query(models.Professional).filter(models.Professional.id == token_data.user_id).first()
