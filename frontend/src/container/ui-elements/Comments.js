...
import { Row, Col, Tooltip, Avatar, List, Card } from 'antd';
    ...
    <Card size="small" className="mb-16">
      <div className="d-flex align-items-start">
        ...
        <div className="ml-12 flex-1">
          <div className="mb-8">
            <span className="font-weight-bold mr-8">Han Solo</span>
            <span className="text-gray font-size-12">{moment().format('YYYY-MM-DD HH:mm')}</span>
          </div>
...
function Comments() {
  ...
  const actions = [
    <span key="comment-basic-like">
      ...
      <span className="pl-8 cursor-auto">{likes}</span>
    </span>,
    <span key=' key="comment-basic-dislike"'>
      <Tooltip title="Dislike">
        <DislikeOutlined theme={action === 'disliked' ? 'filled' : 'outlined'} onClick={dislike} />
      </Tooltip>
      <span className="pl-8 cursor-auto">{dislikes}</span>
    </span>,
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ marginLeft: 12, flex: 1 }}>
                    <div style={{ marginBottom: 8 }}>
                      <span style={{ fontWeight: 'bold', marginRight: 8 }}>Han Solo</span>
                      <Tooltip title={moment().format('YYYY-MM-DD HH:mm:ss')}>
                        <span style={{ color: '#999', fontSize: '12px' }}>{moment().fromNow()}</span>
                      </Tooltip>
                    </div>
