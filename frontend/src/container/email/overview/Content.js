    ...
    emails.map((inbox, key) => {
      ...
      return data.push({
        ...
        name: (
          <EmailAuthor>
            <span
              ...
              className={stared ? 'starActive cursor-pointer' : 'starDeactivate cursor-pointer'}
            >
        ...
        content: (
          <EmailHeader>
            <Heading as="h5">
              <NextLink to={`/admin/email/single/${id}`}>{subject}</NextLink>
              <span className={`mail-badge ${key <= 1 ? 'primary' : ''}`}>{type}</span>
            </Heading>
            <p>{textRefactor(body, 10)}</p>
            {attach.length
              ? attach.map(item => (
                  <a key={item} className="btn-attachment" download href={getImageUrl(`static/img/email/${item}`)}>
        ...
        <>
          <div className="email-top-right d-flex align-items-center">
            ...
            <div className="pagination-slider">
              <span className="btn-paging cursor-pointer">
            ...
            <div className="email-extra">
              ...
              <Dropdown
                content={
                  <>
                    <span className="cursor-pointer d-block py-1">
                      <span>Newest</span>
                    </span>
                    <span className="cursor-pointer d-block py-1">
                      <span>Oldest</span>
