...
const DragHandle = sortableHandle(() => (
  <FeatherIcon size={16} className="cursor-pointer text-gray" icon="move" />
));
...
function ToDo() {
  ...
  const dataSource = useMemo(() => {
    ...
    return todoData.map((item, index) => ({
      ...
      index,
      item: (
        <Span key={`item-${item.key}-${index}`} className={selectedRowKeys.includes(index) ? 'todo-title active' : 'todo-title inactive'}>
          {item.item}
        </Span>
      ),
      action: (
        <div key={`action-${item.key}-${index}`} className="todos-action">
          <DragHandle />
          <span
            className={item.favorite ? 'star active cursor-pointer' : 'star cursor-pointer'}
            onClick={() => dispatch(onStarUpdate(todoData, item.key))}
          >
            <FeatherIcon icon="star" className={item.favorite ? 'text-gold' : 'text-gray-dark'} size={16} />
          </span>
          <span onClick={() => onHandleDelete(item.key)} className="cursor-pointer">
            <FeatherIcon icon="trash-2" size={16} />
          </span>
        </div>
      ),
    }));
  }, [todoData, selectedRowKeys, dispatch]);
  ...
  const onSortEnd = ({ oldIndex, newIndex }) => {
    if (oldIndex !== newIndex) {
      const newData = arrayMoveImmutable([].concat(todoData), oldIndex, newIndex).filter((el) => !!el);
